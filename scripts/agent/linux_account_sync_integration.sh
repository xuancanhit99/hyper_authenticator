...
if [[ -z "$ENV_FILE" ||
  ...
  printf '%s\n' \
    'Usage: scripts/agent/linux_account_sync_integration.sh PUBLIC_CONFIG_JSON --allow-isolated-remote-user' >&2
  printf '%s\n' \
    'CẢNH BÁO: suite ghi account sync của isolated Supabase test user.' >&2
  exit 64
fi
...
for command in dbus-run-session gnome-keyring-daemon jq secret-tool xvfb-run; do
  if ! command -v "$command" >/dev/null 2>&1; then
    printf 'Thiếu Linux account-sync integration dependency: %s\n' "$command" >&2
    exit 69
  fi
done
...
if [[ -z ${ACCOUNT_SYNC_TEST_EMAIL:-} || -z ${ACCOUNT_SYNC_TEST_PASSWORD:-} ]]; then
  printf '%s\n' \
    'Thiếu credential của isolated account-sync test user.' >&2
  exit 78
fi
...
' "$ENV_FILE" >/dev/null; then
  printf '%s\n' \
    'Account-sync harness chỉ nhận JSON public config, tuyệt đối không nhận service-role key.' >&2
  exit 78
fi

SANDBOX=$(mktemp -d \
  "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/hyper-auth-linux-account-sync.XXXXXX")
...
test_email=$ACCOUNT_SYNC_TEST_EMAIL
test_password=$ACCOUNT_SYNC_TEST_PASSWORD
unset ACCOUNT_SYNC_TEST_EMAIL ACCOUNT_SYNC_TEST_PASSWORD
...
jq \
  ...
  '. + {
    ALLOW_ACCOUNT_SYNC_REMOTE_TEST_MUTATION: true,
    ACCOUNT_SYNC_TEST_EMAIL: $email,
    ACCOUNT_SYNC_TEST_PASSWORD: $password
...
export HYPER_AUTH_ACCOUNT_SYNC_TEST_CONFIG="$test_config"
...
if [[ -n ${SUPABASE_SERVICE_ROLE_KEY:-} || -n ${SERVICE_ROLE_KEY:-} ||
  -n ${ACCOUNT_SYNC_TEST_EMAIL:-} || -n ${ACCOUNT_SYNC_TEST_PASSWORD:-} ]]; then
...
probe_attribute='hyper-authenticator-linux-account-sync-integration'
clear_probe() {
...
printf 'test-only' | secret-tool store \
  --label='Hyper Authenticator account-sync CI probe' \
...
xvfb-run --auto-servernum --server-args='-screen 0 1280x1024x24' \
  flutter test integration_test/account_sync_smoke_test.dart \
  --device-id linux \
  --dart-define-from-file="$HYPER_AUTH_ACCOUNT_SYNC_TEST_CONFIG"
SESSION

printf '%s\n' \
  'Linux account-sync pass: upload, new-device download và tombstone.'
