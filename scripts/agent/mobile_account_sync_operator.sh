...
if [[ -z "$CLIENT_ENV_FILE" || -z "$OPERATOR_ENV_FILE" ||
  ...
  printf '%s\n' \
    'Usage: scripts/agent/mobile_account_sync_operator.sh CLIENT_ENV OPERATOR_ENV DEVICE_ID --allow-isolated-user-and-emulator-vault-reset' >&2
  printf '%s\n' \
...
for command in curl dart jq openssl; do
  command -v "$command" >/dev/null 2>&1 || {
    printf 'Thiếu mobile account-sync operator dependency: %s\n' "$command" >&2
    exit 69
  }
done
...
operator_tmp=$(mktemp -d \
  "${TMPDIR:-/tmp}/hyper-auth-mobile-account-sync-operator.XXXXXX")
...
suffix="$(date +%s)-$$"
test_email="account-sync-mobile-${suffix}@example.invalid"
...
if [[ -z "$user_id" ]]; then
  printf '%s\n' 'Supabase không trả ID cho isolated mobile account-sync user.' >&2
  exit 1
fi
rm -f "$create_response"
printf '%s\n' 'MOBILE_ACCOUNT_SYNC_PHASE=isolated-user-created'

runtime_status=0
ACCOUNT_SYNC_TEST_EMAIL="$test_email" ACCOUNT_SYNC_TEST_PASSWORD="$test_password" \
...
if [[ "$delete_status" != 200 && "$delete_status" != 204 ]]; then
  printf 'Không xóa được isolated mobile account-sync user (HTTP %s).\n' \
    "$delete_status" >&2
  exit 1
fi
if [[ "$verify_status" != 404 ]]; then
  printf 'Isolated mobile account-sync user còn tồn tại sau cleanup (HTTP %s).\n' \
    "$verify_status" >&2
  exit 1
fi
user_id=
printf '%s\n' 'MOBILE_ACCOUNT_SYNC_PHASE=remote-cleanup-verified'

if [[ $runtime_status -ne 0 ]]; then
  printf 'Mobile account-sync runtime thất bại với exit code %s.\n' \
    "$runtime_status" >&2
  exit "$runtime_status"
fi

printf '%s\n' \
  'Mobile account-sync operator pass: isolated user lifecycle và runtime đều sạch.'
