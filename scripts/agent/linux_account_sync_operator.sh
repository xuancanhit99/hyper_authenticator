...
if [[ -z "$CLIENT_ENV_FILE" || -z "$OPERATOR_ENV_FILE" ||
  ...
  printf '%s\n' \
    'Usage: scripts/agent/linux_account_sync_operator.sh CLIENT_ENV OPERATOR_ENV --allow-isolated-remote-user' >&2
  printf '%s\n' \
...
for command in curl jq openssl; do
  if ! command -v "$command" >/dev/null 2>&1; then
    printf 'Thiếu account-sync operator dependency: %s\n' "$command" >&2
    exit 69
  fi
done
...
operator_tmp=$(mktemp -d \
  "${TMPDIR:-/tmp}/hyper-auth-account-sync-operator.XXXXXX")
...
suffix="$(date +%s)-$$"
test_email="account-sync-linux-${suffix}@example.invalid"
...
if [[ -z "$user_id" ]]; then
  printf '%s\n' 'Supabase không trả ID cho isolated account-sync test user.' >&2
  exit 1
fi
find "$create_response" -maxdepth 0 -delete
printf '%s\n' 'ACCOUNT_SYNC_OPERATOR_PHASE=isolated-user-created'
...
if [[ "$delete_status" != 200 && "$delete_status" != 204 ]]; then
  printf 'Không xóa được isolated account-sync test user (HTTP %s).\n' \
    "$delete_status" >&2
  exit 1
fi
if [[ "$verify_status" != 404 ]]; then
  printf 'Isolated account-sync test user còn tồn tại sau cleanup (HTTP %s).\n' \
    "$verify_status" >&2
  exit 1
fi
user_id=
printf '%s\n' 'ACCOUNT_SYNC_OPERATOR_PHASE=remote-cleanup-verified'

if [[ $runtime_status -ne 0 ]]; then
  printf 'Linux account-sync client runtime thất bại với exit code %s.\n' \
    "$runtime_status" >&2
  exit "$runtime_status"
fi

printf '%s\n' \
  'Linux account-sync operator pass: isolated user lifecycle và client runtime đều sạch.'
