...
if [[ -z "$ENV_FILE" || -z "$DEVICE_ID" ||
  ...
  printf '%s\n' \
    'Usage: scripts/agent/mobile_account_sync_integration.sh ENV DEVICE_ID --allow-emulator-vault-reset' >&2
  exit 64
fi
if [[ ! -f "$ENV_FILE" ]]; then
...
if [[ -z ${ACCOUNT_SYNC_TEST_EMAIL:-} || -z ${ACCOUNT_SYNC_TEST_PASSWORD:-} ]]; then
  printf '%s\n' 'Thiếu isolated account-sync test-user credential.' >&2
  exit 78
fi
...
sandbox=$(mktemp -d \
  "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/hyper-auth-mobile-account-sync.XXXXXX")
...
chmod 0700 "$sandbox"
umask 077
printf '%s' "$ACCOUNT_SYNC_TEST_EMAIL" >"$email_input"
printf '%s' "$ACCOUNT_SYNC_TEST_PASSWORD" >"$password_input"
...
printf 'Mobile account-sync integration pass: %s (%s).\n' \
