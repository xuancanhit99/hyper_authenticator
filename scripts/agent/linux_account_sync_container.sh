...
if [[ -z "$ENV_FILE" ||
  ...
  printf '%s\n' \
    'Usage: scripts/agent/linux_account_sync_container.sh ENV_FILE --allow-isolated-remote-user' >&2
  printf '%s\n' \
    'Cần export ACCOUNT_SYNC_TEST_EMAIL và ACCOUNT_SYNC_TEST_PASSWORD của isolated test user.' >&2
  exit 64
fi
...
if ! docker info >/dev/null 2>&1; then
  printf '%s\n' 'Docker daemon là bắt buộc cho Linux account-sync runtime.' >&2
  exit 69
fi
...
config_dir=$(mktemp -d \
  "${TMPDIR:-/tmp}/hyper-auth-linux-account-sync-config.XXXXXX")
...
bash scripts/agent/list_working_tree_source_files.sh |
  ...
  docker run --rm --interactive \
    --env CI=true \
    --env ACCOUNT_SYNC_TEST_EMAIL \
    --env ACCOUNT_SYNC_TEST_PASSWORD \
...
printf '%s\n' \
  'Linux container account-sync runtime pass; operator vẫn phải xóa isolated user ở server.'
