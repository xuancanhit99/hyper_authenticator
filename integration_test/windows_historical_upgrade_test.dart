...
const _allowHistoricalVaultMutation = bool.fromEnvironment(
  'ALLOW_WINDOWS_HISTORICAL_VAULT_MUTATION',
);
const _expectedAccount = AuthenticatorAccount(
  id: '00000000-0000-4000-8000-000000000009',
  issuer: 'TEST_ONLY Historical Windows',
...
void main() {
  ...
  testWidgets(
    'bản hiện tại đọc và nâng cấp vault thật từ 1.0.0+9',
    (tester) async {
      expect(
        _allowHistoricalVaultMutation,
        isTrue,
        reason: 'Chỉ chạy qua Windows historical-upgrade harness có opt-in.',
      );

      await app.main();
      await _pumpUntil(tester, find.text(_expectedAccount.issuer));
      debugPrint('WINDOWS_HISTORICAL_PHASE=current-app-visible');
      ...
      debugPrint('WINDOWS_HISTORICAL_PHASE=v2-migration-verified');

      await storage.deleteAll();
      expect(await storage.readAll(), isEmpty);
      debugPrint('WINDOWS_HISTORICAL_PHASE=cleanup-verified');
    },
