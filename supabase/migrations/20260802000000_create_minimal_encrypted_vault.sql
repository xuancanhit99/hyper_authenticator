...
-- Breaking reset được ADR-0019 chấp nhận. Migration này vừa dọn protocol cũ
-- trên deployment hiện hữu, vừa là baseline duy nhất cho database mới.
do $$
declare
  routine record;
begin
  for routine in
    ...
    where n.nspname in ('public', 'private')
      and p.proname = any (array[
        ...
        'register_current_authenticator_device',
        'list_authenticator_device_sessions',
        'revoke_authenticator_device_session',
        'begin_authenticator_device_key_enrollment',
        'list_authenticator_device_keys',
        'publish_authenticator_device_key_wrap',
        'confirm_current_authenticator_device_key',
...
drop table if exists public.synced_accounts cascade;
drop table if exists public.authenticator_device_key_wraps cascade;
drop table if exists public.authenticator_device_keys cascade;
drop table if exists public.authenticator_device_sessions cascade;
drop table if exists private.encrypted_vault_membership_verifiers cascade;
...
create table public.encrypted_vault_snapshots (
  ...
  wrapped_key_nonce text not null
    check (char_length(wrapped_key_nonce) between 16 and 32),
  wrapped_key_ciphertext text not null
    check (char_length(wrapped_key_ciphertext) between 40 and 128),
  wrapped_key_auth_tag text not null
    check (char_length(wrapped_key_auth_tag) between 20 and 32),
...
grant select, delete on public.encrypted_vault_snapshots to authenticated;
...
create policy encrypted_vault_delete_own
  on public.encrypted_vault_snapshots
  for delete
  to authenticated
  using ((select auth.uid()) = user_id);
...
comment on table public.encrypted_vault_snapshots is
  'Minimal E2EE current snapshot; server stores ciphertext and wrapped DEK only.';
comment on function public.publish_encrypted_vault_snapshot is
  'CAS publish for one encrypted snapshot per authenticated user.';
